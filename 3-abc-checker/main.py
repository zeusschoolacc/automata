...
def checkABCCount(text: str) -> str:
    ...
    prev_letter = chr(ord('a') - 1) # set prev_letter to one character before 'a' to keep order
    ...
    for c in text:
        ...
        if character != prev_letter and ord(character) != ord(prev_letter) + 1:
            return False # return False if there is a character that is skipped in order
        
        letter_count[character] += 1
        prev_letter = character

    return letter_count['a'] == letter_count['b'] and letter_count['b'] == letter_count['c']
